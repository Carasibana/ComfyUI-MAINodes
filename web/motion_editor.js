// H3 Motion Editor — DAW-style timeline + mask painter for the Motion Lab.
// Edits the H3MotionEditor node's serialized state (see the node docstring
...
class MotionEditor {
  ...
  // ---------- server data ----------
  onExecuted(msg) {
    if (msg?.h3_paint) this.frames.paint = msg.h3_paint;
    if (msg?.h3_strip) this.frames.strip = msg.h3_strip;
    if (msg?.h3_profile) this.profile = msg.h3_profile;
...
app.registerExtension({
  name: "MAINodes.MotionEditor",
  async beforeRegisterNodeDef(nodeType, nodeData) {
    if (nodeData.name !== "H3MotionEditor") return;
